// UC-7B: Income Summary — category → sub-category → payee breakdown
...
export async function GET(req: NextRequest) {
  ...
  const transactions = readTransactions().filter((t) => {
    ...
    if (!t.isIncome) return false;                        // income only
    if (from && t.date < from) return false;
    if (to   && t.date > to)   return false;
    return true;
  });

  const totalIncome = transactions.reduce((s, t) => s + Math.abs(t.amount), 0);
  ...
  for (const [category, subMap] of catMap) {
    ...
    categories.push({
      category,
      amount: catAmount,
      percentage: totalIncome > 0 ? (catAmount / totalIncome) * 100 : 0,
      subCategories,
    });
  }
  ...
  return NextResponse.json({ categories, totalIncome } satisfies IncomeSummaryResponse);
}
